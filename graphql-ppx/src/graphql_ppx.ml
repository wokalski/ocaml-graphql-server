...
let select_field typ field_name =
  ...
  | Introspection.Object o ->
      begin try
          List.find (fun (field : Introspection.field) -> field.name = field_name) o.fields
        with Not_found ->
          failwithf "Invalid field `%s` for object type `%s`" field_name o.name
      end
  | Introspection.Interface i ->
      begin try
          List.find (fun (field : Introspection.field) -> field.name = field_name) i.fields
        with Not_found ->
          failwithf "Invalid field `%s` for interface type `%s`" field_name i.name
      end
...
let option_map f : Parsetree.expression =
  [%expr function | None -> None | Some x -> Some ([%e f] x)]
    ...
    | Introspection.Type type_name ->
        begin match List.find (match_type_name type_name) ctx.types with
          ...
          | Introspection.Enum e ->
              ...
              [%expr json |> to_string_option |> [%e option_map convert_expr]]
